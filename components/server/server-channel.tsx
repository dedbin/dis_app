...
export const ServerChannel = ({channel, server, role}: ServerChannelProps) => {
  ...
  return (
    <button
        ...
        <p className={cn("line-clamp-1 font-semibold text-sm text-zinc-500 group-hover:text-zinc-200 dark:text-zinc-400 dark:group-hover::text-zinc-300 transition",
      ...
      {channel.name !== "general" && role !== MemberRole.GUEST && (
        <div className="ml-auto flex items-center gap-x-2">
          ...
          <Edit onClick={() => onOpen('editChannel', {server, channel})} className="h-4 w-4 hidden group-hover:block text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 transition"/>
          </ActionTooltip>
